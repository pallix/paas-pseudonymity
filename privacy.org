...
proposal must be kept. This allows the authorization component to
function properly and denies edit rights to all users but the creator.
Thus, in order to keep the system user-friendly, only weak anonymity
can be provided.
